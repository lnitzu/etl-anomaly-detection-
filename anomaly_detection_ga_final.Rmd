...
set.seed(42)
rdata <- data.frame(
  CrtRowCount = round(runif(100, 45000, 85000), 0),
  TimeStamp = seq(as.POSIXct("2024-01-01"), by="2 days", length.out=100)
)

# Inject controlled pipeline failures (Anomalies)
rdata$CrtRowCount[15] <- 13980  # Low level dropout
rdata$CrtRowCount[42] <- 110287 # Extreme peak surge
rdata$CrtRowCount[78] <- 106365 # Secondary peak surge
...
```{r validation_visualization, fig.height=3.8, fig.width=6, fig.align='center'}
